...
export default function CanteenFeeRecord() {
  ...
  const [studentCanteenFeeData, setStudentCanteenFeeData] = useState([]);
  const [loadingCanteenFee, setLoadingCanteenFee] = useState(false);
  ...
  /* ---------- FETCH STUDENTS ---------- */
  useEffect(() => {
    // Always fetch students when component mounts or path changes
    if (location.pathname === "/admin/canteen-fee") {
      fetchStudents();
    }
  }, [location.pathname]);
  ...
  /* ================= FETCH STUDENT CANTEEN FEE DETAILS ================= */
  const fetchStudentCanteenFeeDetails = async (studentId) => {
    setLoadingCanteenFee(true);
    try {
      // Fetch student info
      ...
      // Fetch canteen fee data
      console.log("Fetching canteen fee data...");
      try {
        const canteenFeeRes = await axios.get(`http://localhost:5000/api/canteen-fees/student/${studentId}`);
        setStudentCanteenFeeData(canteenFeeRes.data);
        console.log("Canteen fee data from API:", canteenFeeRes.data);
      } catch (apiError) {
        console.log("Using mock data, API endpoint not available", apiError);
        // Try alternative endpoint
        try {
          const altRes = await axios.get(`http://localhost:5000/api/students/${studentId}/canteen-fees`);
          setStudentCanteenFeeData(altRes.data);
        } catch (altError) {
          console.log("Using mock data");
          const mockCanteenFeeData = [
            {
              _id: "1",
              month: "January 2024",
              year: 2024,
      ...
      // DON'T navigate - just update state to show details view
    } catch (error) {
      console.error("Failed to fetch canteen fee details", error);
      alert("Failed to load canteen fee details: " + error.message);
    } finally {
      setLoadingCanteenFee(false);
    }
  };

  /* ================= ADD NEW CANTEEN FEE RECORD ================= */
  ...
  const processPayment = async () => {
    ...
    try {
      ...
      const response = await axios.put(`http://localhost:5000/api/canteen-fees/${selectedRecord._id}`, updatedRecord);
      
      // Update local state with response data
      setStudentCanteenFeeData(prev => 
        prev.map(record => 
  ...
  /* ================= DELETE CANTEEN FEE RECORD ================= */
  const handleDeleteCanteenFeeRecord = async (recordId) => {
    const confirmDelete = window.confirm("Are you sure you want to delete this canteen fee record?");
    if (!confirmDelete) return;

    try {
      await axios.delete(`http://localhost:5000/api/canteen-fees/${recordId}`);
      
      setStudentCanteenFeeData(prev => prev.filter(record => record._id !== recordId));
      alert("Canteen fee record deleted successfully!");
    } catch (error) {
      console.error("Delete failed:", error);
      alert("Failed to delete canteen fee record");
    }
  };

  /* ================= EDIT CANTEEN FEE RECORD ================= */
  const handleEditCanteenFeeRecord = (record) => {
    setSelectedRecord(record);
  ...
  /* ================= UPDATE CANTEEN FEE RECORD ================= */
  const handleUpdateCanteenFeeRecord = async (e) => {
    e.preventDefault();
    ...
    try {
      if (selectedRecord) {
        ...
        const response = await axios.put(`http://localhost:5000/api/canteen-fees/${selectedRecord._id}`, finalFormData);
        ...
        alert("Canteen fee record updated successfully!");
      } else {
        // Create new record
        const response = await axios.post("http://localhost:5000/api/canteen-fees", finalFormData);
        
        // Add new record to the list
        const newRecord = response.data;
        setStudentCanteenFeeData(prev => [newRecord, ...prev]);
        alert("Canteen fee record added successfully!");
      }
      ...
      alert("Failed to save canteen fee record: " + (error.response?.data?.message || error.message));
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      ...
      alert("Failed to add canteen fee record: " + (error.response?.data?.message || error.message));
    } finally {
      setSaving(false);
    }
  };
    ...
    return (
      <div className="bg-white rounded-xl shadow p-6 mb-6">
        <div className="flex justify-between items-center mb-6">
          <h3 className="text-xl font-bold text-gray-800">
            {selectedRecord ? "Edit Canteen Fee Record" : "Add New Canteen Fee Record"}
          </h3>
          <button
            onClick={() => {
              setShowAddForm(false);
  ...
  const renderPaymentSummary = () => {
    ...
    const pendingPayments = studentCanteenFeeData.filter(record => 
    ...
    return (
      <div className="bg-white rounded-xl shadow p-6 mb-6">
        ...
        <div className="space-y-6">
            ...
            <div className="bg-blue-50 p-4 rounded-lg">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-blue-600">{studentCanteenFeeData.length}</p>
                </div>
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-green-600">
                    ₹{studentCanteenFeeData.reduce((sum, record) => sum + (record.paidAmount || 0), 0).toLocaleString()}
                  </p>
                </div>
          ...
          {/* Records Table */}
          {studentCanteenFeeData.length > 0 ? (
            <div className="overflow-x-auto">
              <table className="w-full">
                ...
                <tbody>
                  {studentCanteenFeeData.map((record) => (
          ...
          <div className="border-t pt-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              {paymentMethods.map((method) => {
                const methodRecords = studentCanteenFeeData.filter(record => 
  ...
  /* ---------- RENDER STUDENT CANTEEN FEE DETAILS ---------- */
  const renderStudentCanteenFeeDetails = () => {
    if (loadingCanteenFee) {
      return (
        <div className="flex items-center justify-center h-64">
          <div className="text-center">
            <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-blue-600 mx-auto mb-4"></div>
            <p className="text-gray-600">Loading canteen fee details...</p>
          </div>
        </div>
      );
    }
    ...
    // Calculate totals
    const totalRecords = studentCanteenFeeData.length;
    const totalPaid = studentCanteenFeeData.reduce((sum, record) => sum + (record.paidAmount || 0), 0);
    const totalDue = studentCanteenFeeData.reduce((sum, record) => sum + (record.dueAmount || 0), 0);
    const totalAmount = studentCanteenFeeData.reduce((sum, record) => sum + (record.totalAmount || 0), 0);
    const paidRecords = studentCanteenFeeData.filter(record => record.status === "Paid").length;
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div className="flex items-center">
            ...
            <h2 className="text-2xl font-bold text-gray-800">Canteen Fee Details</h2>
          </div>
          ...
          <div className="flex flex-col md:flex-row gap-2">
            {!showAddForm && !showPaymentSummary && (
              <>
                <button 
                  onClick={handleAddCanteenFeeRecord}
                  className="bg-green-500 hover:bg-green-600 text-white px-4 py-2 rounded-lg flex items-center justify-center space-x-2"
                >
                  <FaPlus />
                  ...
                  studentCanteenFeeData.map((record) => (
                    <tr key={record._id} className="border-t hover:bg-gray-50">
                      ...
                      <td className="py-3 px-4">
                        <div className="flex space-x-2">
                          <button 
                            onClick={() => handleEditCanteenFeeRecord(record)}
                          ...
                          <button
                            onClick={() => handleDeleteCanteenFeeRecord(record._id)}
    ...
    <div className="flex h-screen bg-gray-100 overflow-hidden">
      ...
      <main className="flex-1 flex flex-col">
        ...
        <section className="flex-1 p-6 overflow-auto">
          {/* Check current view - FIXED: Show student details if selectedStudent is set */}
          {selectedStudent ? renderStudentCanteenFeeDetails() : 
           location.pathname === "/admin/canteen-fee" ? renderStudentsContent() : <Outlet />}
        </section>
      </main>
